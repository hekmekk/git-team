...
@test "use case: (scope: repo-local) an existing global git hook should be respected instead of a repo-local one - commit-msg" {
	...
	git config --local core.hooksPath "/tmp/non-git-team-hooks"
...
@test "use case: (scope: repo-local) an existing global git hook should be respected instead of a repo-local one - prepare-commit-msg" {
...
@test "use case: (scope: repo-local) when git-team is enabled, an amended commit message should have co-authors injected" {
	/usr/local/bin/git-team disable
	echo '# some-repository' > README.md
	git add README.md
	git commit -m 'initial commit'
