...
@test "git-team: disable should persist the current status to status file" {
	mkdir -p /root/.config/git-team/

	run bash -c "/usr/local/bin/git-team disable &>/dev/null && cat /root/.config/git-team/status.toml"
	assert_success
	assert_line 'status = "disabled"'
