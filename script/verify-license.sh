...
done < <( find main.go src/ -type f -exec grep -zvq "${copyright_notice_pattern}" {} \; -exec printf "error: bad/missing copyright notice in file '%s'\n" {} \; )
...
while IFS= read -r error_line; do
  error_lines+=( "${error_line}" )
done < <( find main.go src/ -type f -exec grep -zvq "${license_notice_pattern}" {} \; -exec printf "error: bad/missing license notice in file '%s'\n" {} \; )
